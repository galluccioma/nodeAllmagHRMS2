...
export default function DocumentUpload({ onClose, onSuccess, documentId, initialTitle, initialDescription }: DocumentUploadProps) {
  ...
  const fetchDocumentVisibility = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.departmentIds?.length > 0) {
          ...
          setSelectedDepartments(data.departmentIds)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append("departmentIds", JSON.stringify(visibilityType === "departments" ? visibilityIds : []))
      formData.append("userIds", JSON.stringify(visibilityType === "users" ? visibilityIds : []))
      ...
      if (response.ok) {
        ...
        setError(data.error || `Errore nel ${documentId ? "aggiornamento" : "caricamento"} del documento`)
      }
    } catch (error) {
  ...
  return (
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{documentId ? "Modifica Documento" : "Carica Documento"}</DialogTitle>
          <DialogDescription>
            {documentId ? "Modifica il documento e i permessi di visibilità" : "Carica un nuovo documento e imposta i permessi di visibilità"}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <Label htmlFor="file">File {!documentId && "*"}</Label>
            <Input 
              id="file" 
              type="file" 
            ...
            <Button type="submit" disabled={loading}>
              {loading ? (
                ...
                <>
                  ...
                  {documentId ? "Modifica Documento" : "Carica Documento"}
                </>
              )}
            </Button>
          </div>
