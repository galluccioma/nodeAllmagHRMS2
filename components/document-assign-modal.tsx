...
export default function DocumentAssignModal({ documentId, isOpen, onClose, onSuccess }: DocumentAssignModalProps) {
  ...
  const fetchCurrentVisibility = async () => {
    ...
    try {
      ...
      const deptResponse = await fetch(`/api/admin/documents/${documentId}/visibility/departments`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      if (deptResponse.ok) {
      ...
      const userResponse = await fetch(`/api/admin/documents/${documentId}/visibility/users`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      if (userResponse.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/admin/documents/assign", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        setError(data.error || "Errore nell'assegnazione del documento")
      }
    } catch (error) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Assegna Documento</DialogTitle>
          <DialogDescription>Modifica i permessi di visibilità per questo documento</DialogDescription>
        </DialogHeader>
