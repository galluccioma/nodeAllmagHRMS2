...
export default function NoteCreate({ onClose, onSuccess, noteId, initialTitle, initialContent }: NoteCreateProps) {
  ...
  const fetchNoteVisibility = async () => {
    if (!noteId) return
    ...
    try {
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          title,
          content,
          departmentIds: visibilityType === "departments" ? visibilityIds : [],
          userIds: visibilityType === "users" ? visibilityIds : [],
        }),
      })
      ...
      if (response.ok) {
        ...
        setError(data.error || `Errore nella ${noteId ? "modifica" : "creazione"} della nota`)
      }
    } catch (error) {
  ...
  return (
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{noteId ? "Modifica Nota" : "Crea Nota"}</DialogTitle>
          <DialogDescription>
            {noteId ? "Modifica la nota e i permessi di visibilità" : "Crea una nuova nota e imposta i permessi di visibilità"}
          </DialogDescription>
        </DialogHeader>
            ...
            <Button type="submit" disabled={loading}>
              {loading ? (
                ...
                <>
                  ...
                  {noteId ? "Modifica Nota" : "Crea Nota"}
                </>
              )}
            </Button>
          </div>
